...
class LinearSweepFrequencyJamming(BaseSignal):
    ...
    def get_signal(self):
        ...
        K_FM = 2 * numpy.pi * band*random.uniform(0.5, 1.5) *0.4/ t[-1]**2

        # Generate linear sweep frequency
        lsfj = U_0 * numpy.exp(1j * (2 * numpy.pi * f_0 * t + 2 * numpy.pi * K_FM * t**3))
...
if __name__ == '__main__':
    ...
    # plt.savefig('nsftdomain.jpg', format='jpg')
    # plt.savefig('nsftdomain.eps', format='eps', dpi=300)

    signal.f_domain = fft.fftshift(fft.fft(signal.time_domain))
    plt.figure(2)
    plt.plot(signal.f_range, numpy.abs(signal.f_domain))
    # plt.savefig('nsffdomain.jpg', format='jpg')
    # plt.savefig('nsffdomain.eps', format='eps', dpi=300)
    ...
    # plt.savefig('nsftfdomain.jpg', format='jpg')
    # plt.savefig('nsftfdomain.eps', format='eps', dpi=300)
