...
class Interruptsampledirect:
    def para_init(self, origin_signal):
        ...
        sampleinter = 0.2*t_duration/samplenum
        samplegap = 0.8*t_duration/samplenum
        ...
        for i in range(samplenum):
            ...
            end_index = start_index +sample_index
            start_direct_index = end_index
            end_direct_index = end_index + sample_index *repeat
            ...
            for j in range(repeat):
                startnum = (start_direct_index+j*sample_index)
                endnum = (start_direct_index+(j+1)*sample_index)
                self.interfered_signal[startnum:endnum] += sampled_signal
...
if __name__ == '__main__':
    ...
    plt.figure(5)
    plt.plot(signal.origin_signal.t_range, signal.interfered_signal.imag)

    # plt.savefig('isrtdomain.jpg', format='jpg')
    # plt.savefig('isrtdomain.eps', format='eps', dpi=300)

    signal.f_domain = fft.fftshift(fft.fft(signal.interfered_signal))
    plt.figure(2)
    plt.plot(signal.origin_signal.f_range, numpy.abs(signal.f_domain.real))
    plt.figure(6)
    plt.plot(signal.origin_signal.f_range, numpy.abs(signal.f_domain.imag))
    # plt.savefig('isrfdomain.jpg', format='jpg')
    # plt.savefig('isrfdomain.eps', format='eps', dpi=300)
    ...
    # plt.savefig('isrtfdomain.jpg', format='jpg')
    # plt.savefig('isrtfdomain.eps', format='eps', dpi=300)
